...
// 秘银锻造厂
public class MithrilForgeManager : MonoBehaviour
{
    ...
    string resourceName = "秘银锻造厂";
    string resourceDescription = "秘银锻造厂会自动消耗材料不断的进行制造秘银";
    int resourceQuantity = 0;

    string btnText = "建造";


    double baseYield = 0.008;//纳米材料基础产量

    Dictionary<ResourceType, double> depleted;//每个成品消耗的资源
    readonly FacilityType type = FacilityType.MithrilForge;
    ...
    {
        //获取到纳米材料的制作配方
        depleted = ResourceManager.Instance.formula[ResourceType.Mithril];
    ...
    void Output()
    {
        ...
        rmb *= ResourceAdditionManager.Instance.GetMithrilUp();//加上专项加成的提升
        ...
        facilityPanelManager.UpdateOutPut(ResourceType.Mithril, secondCount, false);//更新产出资源
