...
public class CopperMinerManager : MonoBehaviour
{
    ...
    readonly FacilityType type = FacilityType.CopperMiner;
    ...
    void Output()
    {
        ...
        {
            double count = baseYield * facilityPanelManager.GetCount();//每秒产出铜矿


            count *= ResourceAdditionManager.Instance.GetMiningWorkerUp(); //加上采矿工人的提升


            count *= ResourceAdditionManager.Instance.GetToolUp();//加上工具对铜矿工人的提升

            count *= ResourceAdditionManager.Instance.GetWorkerUp();//加上矿洞员工加成的提升

            count *= ResourceAdditionManager.Instance.GetMineCopperUp();//加上铜矿专项加成
